...
void Engine_Interface::handle_drag_and_drop(string file){
    if(is_window_on_top(get_window("edit_seeds"))){
        File_IO_Load load(file);

        if(load.file_loaded()){
            game.world.set_seeds(load.get_data());
        }
    }
    else if(is_window_on_top(get_window("cipher_text"))){
        File_IO_Load load(file);

        if(load.file_loaded()){
            game.world.set_input_text(load.get_data());
        }
    }
        ...
        if(boost::algorithm::ends_with(file,".png")){
            if(game.world.set_input_image(file)){
                get_window("cipher_image")->toggle_on(true,true);
            }
        }
            ...
            if(load.file_loaded()){
                ...
                get_window("cipher_text")->toggle_on(true,true);
            }
        }
    }
}
